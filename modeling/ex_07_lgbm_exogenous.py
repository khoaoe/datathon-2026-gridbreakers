"""
EX_03: LightGBM with v2 Feature Engineering
- Calendar + Fourier + historical profiles (all available at prediction time)
- Target lags + rolling stats (recursive prediction)
- Auxiliary table patterns indexed by day-of-week / month
- SHAP explainability
"""
import sys
import time
...
from modeling.config import (LGBM_PARAMS, MODEL_DIR, SUBMISSION_DIR,
                              VAL_START, VAL_END, SEED)
...
def recursive_predict(model, train_df, test_dates, feature_cols, profiles, target="Revenue"):
    """
    ...
    for i, date in enumerate(test_dates):
        ...
        # Update history
        history = pd.concat([history, pd.DataFrame({"Date": [pd.Timestamp(date)], target: [pred]})],
                            ignore_index=True)
...
def main():
    ...
    tracker = ExperimentTracker("ex_07_lgbm_exogenous")

    print("=" * 60)
    print("EX_07: LightGBM + EXOGENOUS COVARIATES")
    print("=" * 60)
    ...
    tracker.log_params({"n_features": len(feature_cols), "n_train": len(trn), "n_val": len(val)})
    ...
    tracker_cogs = ExperimentTracker("ex_07_lgbm_exogenous_cogs")
    ...
    # ── Save models ──────────────────────────────────────────────────
    with open(MODEL_DIR / "ex_07_lgbm_exogenous_rev.pkl", "wb") as f:
        pickle.dump(model_rev, f)
    with open(MODEL_DIR / "ex_07_lgbm_exogenous_cogs.pkl", "wb") as f:
    ...
    try:
        ...
        importance.to_csv(MODEL_DIR / "ex_07_shap_importance.csv", index=False)
    ...
    make_submission(test["Date"], rev_preds, cogs_preds,
                    SUBMISSION_DIR / "ex_07_lgbm_exogenous.csv")
