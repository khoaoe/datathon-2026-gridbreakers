"""
EX_19: Robust dual-weight ensemble deep research.

Purpose:
- Extend EX_18 with deeper ensemble controls and broader fold coverage.
- Optimize separate global weights for Revenue and COGS using recursive OOF
  predictions from multiple yearly folds.
- Add robustness penalty to reduce fold-instability and over-concentration.
- Generate a direct candidate plus low-drift bridges from EX_18 anchor.

Outputs:
- output/tracking/ex_19_robust_dual_weight_ensemble/fold_component_scores.csv
- output/tracking/ex_19_robust_dual_weight_ensemble/fold_weight_search.csv
- output/tracking/ex_19_robust_dual_weight_ensemble/fold_global_metrics.csv
- output/tracking/ex_19_robust_dual_weight_ensemble/global_weight_search.csv
- output/tracking/ex_19_robust_dual_weight_ensemble/ensemble_summary.csv
- output/tracking/ex_19_robust_dual_weight_ensemble/bridge_summary.csv
- output/tracking/ex_19_robust_dual_weight_ensemble/notes.md
- output/tracking/ex_19_robust_dual_weight_ensemble/meta.json
- output/submissions/ex_19_robust_dual_weight_ensemble.csv
- output/submissions/ex_19_bridge_w01.csv .. ex_19_bridge_w04.csv
"""
...
TRACK = Path("output/tracking/ex_19_robust_dual_weight_ensemble")
SUB_DIR = Path("output/submissions")
ANCHOR_PATH = Path("output/submissions/ex_18_ensemble_step.csv")
...
ROBUST_STD_PENALTY = 0.10
WEIGHT_SHRINK_L2 = 0.02
RAND_SEARCH_TRIALS = 8000
...
def _safe_weight_optimize_global(
    ...
    def obj(w):
        ...
        fold_maes = [float(abs_err[fold_ids == f].mean()) for f in uniq_folds]
...
def _make_bridges(candidate_path: Path):
    ...
    for w in [0.01, 0.02, 0.03, 0.04]:
        ...
        tag = int(round(w * 100))
        out_path = SUB_DIR / f"ex_19_bridge_w{tag:02d}.csv"
...
def main() -> None:
    ...
    print("=" * 78)
    print("EX_19: Robust dual-weight ensemble deep research")
    print("=" * 78)
    ...
    feat_full, profiles_full = build_feature_table(
    ...
    candidate_path = SUB_DIR / "ex_19_robust_dual_weight_ensemble.csv"
