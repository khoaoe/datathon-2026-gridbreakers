...
def build_stateless_features(df, profile_df=None):
    ...
    feat_df, profiles = build_feature_table(df, verbose=False, profile_source_df=profile_df)
...
def evaluate_fold(sales: pd.DataFrame, fold: dict):
    ...
    x_trn_trend_rev = scaler_rev.fit_transform(trn[trend_features])
    x_val_trend_rev = scaler_rev.transform(val[trend_features])
    
    trend_model_rev = Ridge(alpha=10.0)
    trend_model_rev.fit(x_trn_trend_rev, trn["Revenue"])
    
    scaler_cogs = StandardScaler()
    x_trn_trend_cogs = scaler_cogs.fit_transform(trn[trend_features])
    x_val_trend_cogs = scaler_cogs.transform(val[trend_features])
    
    trend_model_cogs = Ridge(alpha=10.0)
    trend_model_cogs.fit(x_trn_trend_cogs, trn["COGS"])
    
    # Get trend predictions
    trn_trend_pred_rev = trend_model_rev.predict(x_trn_trend_rev)
    val_trend_pred_rev = trend_model_rev.predict(x_val_trend_rev)
    
    trn_trend_pred_cogs = trend_model_cogs.predict(x_trn_trend_cogs)
    ...
    lgb_rev.fit(
        trn[cols_rev].fillna(0), trn_resid_rev,
    ...
    lgb_cogs.fit(
        trn[cols_cogs].fillna(0), trn_resid_cogs,
...
def main():
    ...
    trend_model_rev = Ridge(alpha=10.0).fit(x_trn_trend_rev, trn["Revenue"])
    
    scaler_cogs = StandardScaler()
    x_trn_trend_cogs = scaler_cogs.fit_transform(trn[trend_features])
    x_tst_trend_cogs = scaler_cogs.transform(tst[trend_features])
    trend_model_cogs = Ridge(alpha=10.0).fit(x_trn_trend_cogs, trn["COGS"])
