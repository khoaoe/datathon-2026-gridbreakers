"""
EX_20: Recency-weighted dual-ensemble deep research.

Purpose:
- Start from EX_19 winner as anchor and focus on recent-regime robustness.
- Optimize separate global weights for Revenue and COGS using recursive OOF
    predictions from 2021/2022 folds with recency weighting.
- Keep low-drift bridge generation from EX_19 production anchor.

Outputs:
- output/tracking/ex_20_recency_weighted_dual_ensemble/fold_component_scores.csv
- output/tracking/ex_20_recency_weighted_dual_ensemble/fold_weight_search.csv
- output/tracking/ex_20_recency_weighted_dual_ensemble/fold_global_metrics.csv
- output/tracking/ex_20_recency_weighted_dual_ensemble/global_weight_search.csv
- output/tracking/ex_20_recency_weighted_dual_ensemble/ensemble_summary.csv
- output/tracking/ex_20_recency_weighted_dual_ensemble/bridge_summary.csv
- output/tracking/ex_20_recency_weighted_dual_ensemble/notes.md
- output/tracking/ex_20_recency_weighted_dual_ensemble/meta.json
- output/submissions/ex_20_recency_weighted_dual_ensemble.csv
- output/submissions/ex_20_bridge_w01.csv .. ex_20_bridge_w04.csv
"""
...
TRACK = Path("output/tracking/ex_20_recency_weighted_dual_ensemble")
SUB_DIR = Path("output/submissions")
ANCHOR_PATH = Path("output/submissions/ex_19_robust_dual_weight_ensemble.csv")
...
def _make_bridges(candidate_path: Path):
    ...
    for w in [0.01, 0.02, 0.03, 0.04]:
        ...
        tag = int(round(w * 100))
        out_path = SUB_DIR / f"ex_20_bridge_w{tag:02d}.csv"
...
def main() -> None:
    ...
    print("=" * 78)
    print("EX_20: Recency-weighted dual-ensemble deep research")
    print("=" * 78)
    ...
    candidate_path = SUB_DIR / "ex_20_recency_weighted_dual_ensemble.csv"
